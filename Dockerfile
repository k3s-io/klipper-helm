...
RUN if [ "${TARGETARCH}" = "arm/v7" ]; then \
    ...
    curl -sL https://get.helm.sh/helm-v3.19.0-linux-${ARCH}.tar.gz | tar xvzf - --strip-components=1 -C /usr/bin
COPY entry /usr/bin/
...
RUN mkdir -p /go/src/github.com/k3s-io/helm-set-status && \
    ...
    go mod edit --replace helm.sh/helm/v3=helm.sh/helm/v3@v3.19.0 && \
    go mod tidy && \
    make install
